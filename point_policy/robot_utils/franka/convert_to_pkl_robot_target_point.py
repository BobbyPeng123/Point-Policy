...
only_empty_space = (object_labels == ["empty space"])
...
for TASK_NAME in (task_names or [p.name for p in PROCESSED_DATA_PATH.iterdir() if p.is_dir()]):
    # DATASET_PATH = Path(f"{PROCESSED_DATA_PATH}/{TASK_NAME}")
    DATASET_PATH = PROCESSED_DATA_PATH / TASK_NAME
    coords_root = Path("coordinates") / TASK_NAME
    need_empty = "empty space" in object_labels          ### NEW

    save_file = SAVE_DATA_PATH / f"{TASK_NAME}.pkl"
    ...
    for i, data_point in enumerate(dirs):
        ...
        print(f"{TASK_NAME}: {i+1}/{len(dirs)}  ->  {data_point.name}")

        # if NUM_DEMOS is not None and int(str(data_point).split("_")[-1]) >= NUM_DEMOS:
        #     print(f"Skipping data point {data_point}")
        #     continue
        if NUM_DEMOS is not None and int(re.search(r'\d+', data_point.name).group()) >= NUM_DEMOS:
            print(f"Skipping data point {data_point.name}")
            break

        # ---------- 若需要 empty space，检查标注 ----------
        if need_empty:
            coord_file = coords_root / f"{data_point.name}.pkl"
            if not coord_file.exists():
                print(f"  ↳ 缺少 {coord_file}, 跳过此 demo")
                continue
            empty_2d = pkl.load(open(coord_file, "rb"))
        ...
        # =========== 手工 empty space -> object_tracks ===========
        # =========== 手工 empty space -> object_tracks ===========
        if need_empty:
            T = len(observation[f"pixels{camera_indices[0]}"])        # 视频总帧数
            for cam in camera_indices:
                cam_name  = f"cam_{cam}"
                pixel_key = camera2pixelkey[cam_name]

                # ------------- 2-D -------------
                xy_one = np.array(empty_2d[cam_name], np.float32)[None, None, ...]  # (1,1,2)
                xy_seq = np.repeat(xy_one, T, axis=0)                               # (T,1,2)

                key2d = f"object_tracks_{pixel_key}"
                if key2d in observation:                                            # 已有其它物体 → 追加
                    observation[key2d] = np.concatenate([observation[key2d], xy_seq], axis=1)
                else:                                                               # 只有 empty space
                    observation[key2d] = xy_seq
            ...
            if not use_gt_depth:
                ...
                for cam in camera_indices:
                    cam_name = f"cam_{cam}"
                    extr = calibration_data[cam_name]["ext"]
                    intr = calibration_data[cam_name]["int"]
                    intr = np.c_[intr, np.zeros((3,1))]
                    P.append(intr @ extr)

                    u256, v256 = empty_2d[cam_name]
                    w0, h0 = original_img_size
                    wc, hc = w0*(crop_w[1]-crop_w[0]), h0*(crop_h[1]-crop_h[0])
                    u0 = u256 / save_img_size[0] * wc + w0*crop_w[0]
                    v0 = v256 / save_img_size[1] * hc + h0*crop_h[0]
                    pts.append(np.array([[u0, v0]], dtype=np.float32))              # (1,2)

                xyz_one = triangulate_points(P, pts)[:, :3][None, None, ...]        # (1,1,3)
                xyz_seq = np.repeat(xyz_one, T, axis=0)                             # (T,1,3)

                for cam in camera_indices:
                    pk   = camera2pixelkey[f"cam_{cam}"]
                    key3d = f"object_tracks_3d_{pk}"
                    if key3d in observation:
                        observation[key3d] = np.concatenate([observation[key3d], xyz_seq], axis=1)
                    else:
                        observation[key3d] = xyz_seq
